...
namespace LW_AskOnline.OracleDAL
{
	/// <summary>
	/// 用参数方式实现数据层示例。
	/// </summary>
    public class SysManage1 : ISysManage//这里必须实现接口，否则工厂类创建接口报错
	{
        ...
        public int AddTreeNode(LW_AskOnline.Model.SysNode model)
		{
        ...
        public void UpdateNode(LW_AskOnline.Model.SysNode model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// <returns></returns>
        public LW_AskOnline.Model.SysNode GetNode(int NodeID)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * from S_Tree ");	
			strSql.Append(" where NodeID=@NodeID");
            ...
            LW_AskOnline.Model.SysNode node = new LW_AskOnline.Model.SysNode();
