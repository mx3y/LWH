...
namespace LW_AskOnline.BLL
{
	...
	{
		private readonly Isys_dict_type dal=DataAccess.Createsys_dict_type();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public LW_AskOnline.Model.sys_dict_type GetModelByCache(decimal type_id)
		{
			
			string CacheKey = "sys_dict_typeModel-" + type_id;
			...
			return (LW_AskOnline.Model.sys_dict_type)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<LW_AskOnline.Model.sys_dict_type> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<LW_AskOnline.Model.sys_dict_type> DataTableToList(DataTable dt)
		{
			List<LW_AskOnline.Model.sys_dict_type> modelList = new List<LW_AskOnline.Model.sys_dict_type>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				LW_AskOnline.Model.sys_dict_type model;
