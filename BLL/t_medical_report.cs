...
namespace LW_AskOnline.BLL
{
	...
	{
		private readonly It_medical_report dal=DataAccess.Createt_medical_report();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public LW_AskOnline.Model.t_medical_report GetModelByCache(decimal report_id)
		{
			
			string CacheKey = "t_medical_reportModel-" + report_id;
			...
			return (LW_AskOnline.Model.t_medical_report)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<LW_AskOnline.Model.t_medical_report> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<LW_AskOnline.Model.t_medical_report> DataTableToList(DataTable dt)
		{
			List<LW_AskOnline.Model.t_medical_report> modelList = new List<LW_AskOnline.Model.t_medical_report>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				LW_AskOnline.Model.t_medical_report model;
